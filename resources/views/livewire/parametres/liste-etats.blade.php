<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white border border-indigo-200 rounded-lg shadow-sm p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">{{ $editId ? 'Modifier l\'état' : 'Nouvel état' }}</h2>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors {{ !$etat->actif ? 'opacity-60' : '' }}">
                    <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $etat->Etat }}</td>
                    <td class="px-6 py-4 text-sm text-gray-500">{{ $etat->CodeEtat ?? '—' }}</td>
                    <td class="px-6 py-4 text-center">
                        <button wire:click="toggleActif({{ $etat->idEtat }})"
                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium transition-colors
                                {{ $etat->actif ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}">
                            <span class="w-2 h-2 rounded-full mr-1.5 {{ $etat->actif ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                            {{ $etat->actif ? 'Actif' : 'Inactif' }}
                        </button>
                    </td>
                    <td class="px-6 py-4 text-right space-x-2">
                        <button wire:click="openEdit({{ $etat->idEtat }})"
                        ...
                        <button wire:click="delete({{ $etat->idEtat }})"
